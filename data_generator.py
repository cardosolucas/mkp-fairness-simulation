...
def completeCheckRankingProperties(_ranking, _protected_group):
    ...
    if not isinstance(_ranking, (list, tuple, np.ndarray)) and not isinstance(_ranking, (str, bytes)):
        raise TypeError("Input ranking must be a list-wise structure defined by '[]' symbol")
    if not isinstance(_protected_group, (list, tuple, np.ndarray)) and not isinstance(_protected_group, (str, bytes)):
    ...
    if pro_N >= user_N:  # check size of protected group
        raise ValueError(f"Please input a protected group with size less than total user {pro_N} >= {user_N}")

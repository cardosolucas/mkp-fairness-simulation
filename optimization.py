...
def calculateEvaluateRez(_rez, _data, _inputscores, _k, _accmeasure):
    ...
    # error handling for input type
    if not isinstance(_rez, (list, tuple, np.ndarray)) and not isinstance(_rez, str):
        raise TypeError("Input parameter list must be a list-wise structure defined by '[]' symbol")
    if not isinstance(_inputscores, (list, tuple, np.ndarray)) and not isinstance(_inputscores, str):
    ...
    if not isinstance(_accmeasure, str):
        raise TypeError(
            "Input accuracy measure must be a string that choose from ['scoreDiff', 'positionDiff', 'kendallDis', 'spearmanDis', 'pearsonDis'] defined in the begining of this file")
...
def calculateEstimateY(_M_nk_x, _inputscores, _clusters, _N, _k, _accmeasure):
    ...
    if _accmeasure == SCORE_DIVERGENCE:
        L_y = calculateScoreDifference(sorted_score_hat, sorted_inputscores)
        ranking_loss = L_y

    elif _accmeasure == POSITION_DIFFERENCE:
        L_y = calculatePositionDifference(per_scores_hat, per_scores_input)
        ranking_loss = L_y

    elif _accmeasure == KENDALL_DIS:
        L_y = calculateKendallDistance(per_scores_hat, per_scores_input)  # kendall distance
        ranking_loss = L_y

    # for spearman and pearson relation, use the negative value to minimize during optimization
    elif _accmeasure == SPEARMAN_COR:
        L_y = calculateSpearmanR(score_hat, _inputscores)
        ranking_loss = -L_y

    elif _accmeasure == PEARSON_COR:
        L_y = calculatePearsonC(score_hat, _inputscores)
        ranking_loss = -L_y
...
def lbfgsOptimize(_params, _data, _pro_data, _unpro_data,
    ...
    if (att_N * pro_att_N * unpro_att_N) == 0:
        raise ValueError(
            "Input data, protected group data, and unprotected group data should have at least one attribute column")
    if att_N != pro_att_N:
